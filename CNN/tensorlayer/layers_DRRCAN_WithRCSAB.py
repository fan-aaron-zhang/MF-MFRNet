...
class Layer(object):
    ...
    def __init__(
        ...
        if (name in set_keep['_layers_name_list']) and False: #name_reuse == False
    ...
    def __init__(
        ...
        with tf.variable_scope(name, reuse=reuse) as vs:
            ...
            if b_init:
                ...
                self.outputs = act( tf.nn.conv2d(self.inputs, W, strides=strides, padding=padding, use_cudnn_on_gpu=use_cudnn_on_gpu, data_format=data_format,name='conv1_') + b)
            else:
                self.outputs = act( tf.nn.conv2d(self.inputs, W, strides=strides, padding=padding, use_cudnn_on_gpu=use_cudnn_on_gpu, data_format=data_format,name='conv2_'))
...
class FeaturePyramidExtractorLayer(Layer):
    """ Feature pyramid extractor Layer"""
    def __init__(
        self,
        num_levels = 6,
        ...
        self.num_levels=num_levels
        #with tf.variable_scope(name, reuse=reuse) as vs:
            #self.convs=[(tf.layers.Conv2D(l, (3, 3), (2, 2), 'same')
    #,tf.layers.Conv2D(l, (3, 3), (1, 1), 'same')) for l in filters]
    #self.convs = [(Conv2d(l, (3, 3), (2, 2), padding='SAME', act=tf.nn.leaky_relu)
    #,Conv2d(l, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu)) for l in filters]
    def __call__(self, layer, reuse):
        """
        Args:
        ...
        for i in range(self.num_levels):
            #x = self.convs[i][0](x)
            with tf.variable_scope('convs', reuse=reuse) as vs:
                x = Conv2d(x, self.filters[i], (3, 3), (2, 2), padding='SAME', act=tf.nn.leaky_relu, reuse=reuse, name='rere2_{}'.format(i))
                #x = tf.nn.leaky_relu(x, 0.1)
                #x = self.convs[i][1](x)
                x = Conv2d(x, self.filters[i], (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, reuse=reuse, name='rere1_{}'.format(i))
            #x = tf.nn.leaky_relu(x, 0.1)

            features_pyramid.append(x)
        self.outputs = features_pyramid[::-1]
        self.all_layers = list(x.all_layers)
        self.all_params = list(x.all_params)
        self.all_drop = dict(x.all_drop)
        self.all_layers.extend([self.outputs])
...
class GlobalPoolLayer(Layer):
    def __init__(
        self,
        layer1 = None, #input 'Feature' with Layer format[batch,height,width,channel],
        layer2 = None, #
        layer3 = None, #
        pool_type = 'average',
        #warp = warp_type
    ...
    #def __call__(self, x, flow):
        ...
        #self.inputs = layer.outputs
        with tf.variable_scope(name) as vs:
             self.outputs = adaptive_global_average_pool_2d(x)
...
class ReduceMeanLayer(Layer):
    ...
    #def __call__(self, x, flow):
        ...
        #self.inputs = layer.outputs
        with tf.variable_scope(name) as vs:
             self.outputs = tf.reduce_mean(x, axis=axis, keepdims=True)
...
class ReduceMaxLayer(Layer):
    ...
    #def __call__(self, x, flow):
        ...
        #self.inputs = layer.outputs
        with tf.variable_scope(name) as vs:
             self.outputs = tf.reduce_max(x, axis=axis, keepdims=True)
...
class CALayer(Layer):
    ...
    #def __call__(self, x, flow):
        ...
        #self.inputs = layer.outputs
        with tf.variable_scope(name) as vs:
             channel = x1.outputs.get_shape()[-1]
             x1 = ReduceMeanLayer(x1, [1, 2])
             assert x1.outputs.get_shape()[1:] == (1, 1, channel)
             x1 = FlattenLayer(x1, name='avg_f_0_{}_{}_{}'.format(i,j,c))
             x1 = DenseLayer(x1, n_units=channel//ratio, act=tf.nn.relu, W_init=w_init, b_init=b_init, reuse=False, name='dense_0_{}_{}_{}'.format(i,j,c))
             x1.outputs = tf.reshape(x1.outputs, [x1.outputs.get_shape()[0], 1, 1, channel//ratio])
             assert x1.outputs.get_shape()[1:] == (1, 1, channel // ratio)
             x1 = FlattenLayer(x1, name='avg_f_1_{}_{}_{}'.format(i,j,c))
             x1 = DenseLayer(x1, n_units=channel, act=tf.identity, W_init=w_init, b_init=b_init, reuse=False, name='dense_1_{}_{}_{}'.format(i,j,c))
             x1.outputs = tf.reshape(x1.outputs, [x1.outputs.get_shape()[0], 1, 1, channel])
             assert x1.outputs.get_shape()[1:] == (1, 1, channel)

             x2 = ReduceMaxLayer(x2, [1, 2])
             assert x2.outputs.get_shape()[1:] == (1, 1, channel)
             x2 = FlattenLayer(x2, name='max_f_0_{}_{}_{}'.format(i, j, c))
             x2 = DenseLayer(x2, n_units = channel//ratio, act=tf.nn.relu, reuse=True, name = 'dense_0_{}_{}_{}'.format(i,j,c))
             x2.outputs = tf.reshape(x2.outputs, [x2.outputs.get_shape()[0], 1, 1, channel // ratio])
             assert x2.outputs.get_shape()[1:] == (1, 1, channel // ratio)
             x2 = FlattenLayer(x2, name='max_f_1_{}_{}_{}'.format(i, j, c))
             x2 = DenseLayer(x2, n_units=channel, act=tf.identity, W_init=w_init, b_init=b_init, reuse=True, name= 'dense_1_{}_{}_{}'.format(i,j,c))
             x2.outputs = tf.reshape(x2.outputs, [x2.outputs.get_shape()[0], 1, 1, channel])
             assert x2.outputs.get_shape()[1:] == (1, 1, channel)

             sum_CA = ElementwiseLayer([x1, x2], tf.add, name='sum_CA_{}_{}_{}'.format(i,j,c))
             sum_CA.outputs = tf.sigmoid(sum_CA.outputs, 'sigmoid')

             sum_CA.outputs = tf.tile(sum_CA.outputs, multiples=[1, layer1.outputs.shape[1], layer1.outputs.shape[2], 1])
             sum_CA = ElementwiseLayer([sum_CA, layer1], tf.multiply, name='sum_multiply_CA_{}_{}_{}'.format(i,j,c))

             self.outputs = sum_CA.outputs

        self.all_layers = list(sum_CA.all_layers)
        self.all_params = list(sum_CA.all_params)
        self.all_drop = dict(sum_CA.all_drop)
        self.all_layers.extend( [self.outputs] )

class CA_WithConv_Layer(Layer):
    def __init__(
        self,
        layer1 = None, #input 'Feature' with Layer format[batch,height,width,channel],
        layer2 = None, #
    ...
    #def __call__(self, x, flow):
        ...
        with tf.variable_scope(name) as vs:
             ...
             x1 = ReduceMeanLayer(x1, [1, 2])
             assert x1.outputs.get_shape()[1:] == (1, 1, channel)
             #x1 = FlattenLayer(x1, name='avg_f_0_{}_{}_{}_{}'.format(i,j,k,c))
             #x1 = DenseLayer(x1, n_units=channel//ratio, act=tf.nn.relu, W_init=w_init, b_init=b_init, reuse=False, name='dense_0_{}_{}_{}_{}'.format(i,j,k,c))
             #x1.outputs = tf.reshape(x1.outputs, [x1.outputs.get_shape()[0], 1, 1, channel//ratio])
             #assert x1.outputs.get_shape()[1:] == (1, 1, channel // ratio)
             #x1 = FlattenLayer(x1, name='avg_f_1_{}_{}_{}_{}'.format(i,j,k,c))
             #x1 = DenseLayer(x1, n_units=channel, act=tf.identity, W_init=w_init, b_init=b_init, reuse=False, name='dense_1_{}_{}_{}_{}'.format(i,j,k,c))
             #x1.outputs = tf.reshape(x1.outputs, [x1.outputs.get_shape()[0], 1, 1, channel])
             #assert x1.outputs.get_shape()[1:] == (1, 1, channel)
             x1 = Conv2d(x1, channel//ratio, (1, 1), (1, 1), padding='SAME', act=tf.nn.leaky_relu, reuse=False, name='dense_0_{}_{}_{}'.format(i,j,c))
             x1 = Conv2d(x1, channel, (1, 1), (1, 1), padding='SAME', act=tf.identity, reuse=False, name='dense_1_{}_{}_{}'.format(i,j,c))


             x2 = ReduceMaxLayer(x2, [1, 2])
             assert x2.outputs.get_shape()[1:] == (1, 1, channel)
             #x2 = FlattenLayer(x2, name='max_f_0_{}_{}_{}_{}'.format(i, j, k, c))
             #x2 = DenseLayer(x2, n_units = channel//ratio, act=tf.nn.relu, reuse=True, name = 'dense_0_{}_{}_{}_{}'.format(i,j,k,c))
             #x2.outputs = tf.reshape(x2.outputs, [x2.outputs.get_shape()[0], 1, 1, channel // ratio])
             #assert x2.outputs.get_shape()[1:] == (1, 1, channel // ratio)
             #x2 = FlattenLayer(x2, name='max_f_1_{}_{}_{}_{}'.format(i, j, k, c))
             #x2 = DenseLayer(x2, n_units=channel, act=tf.identity, W_init=w_init, b_init=b_init, reuse=True, name= 'dense_1_{}_{}_{}_{}'.format(i,j,k, c))
             #x2.outputs = tf.reshape(x2.outputs, [x2.outputs.get_shape()[0], 1, 1, channel])
             #assert x2.outputs.get_shape()[1:] == (1, 1, channel)
             x2 = Conv2d(x2, channel//ratio, (1, 1), (1, 1), padding='SAME', act=tf.nn.leaky_relu, reuse=True, name='dense_0_{}_{}_{}'.format(i, j, c))
             x2 = Conv2d(x2, channel, (1, 1), (1, 1), padding='SAME', act=tf.identity, reuse=True, name='dense_1_{}_{}_{}'.format(i, j, c))
...
class SALayer(Layer):
    ...
    #def __call__(self, x, flow):
        ...
        #self.inputs = layer.outputs
        with tf.variable_scope(name) as vs:
             x1 =ReduceMeanLayer(x1, 3)
             assert x1.outputs.get_shape()[-1] == 1
             x2 = ReduceMaxLayer(x2, 3)
             assert x2.outputs.get_shape()[-1] == 1
             concat = ConcatLayer([x1, x2], concat_dim=3, name='concat_SA_{}_{}_{}'.format(i,j,c))
             assert concat.outputs.get_shape()[-1] == 2
             concat = Conv2d(concat, 1, (kernel_size_SA, kernel_size_SA), (1, 1), act=tf.nn.sigmoid, padding='SAME', W_init=w_init, name='convolution_SA_{}_{}_{}'.format(i,j,c))
             assert concat.outputs.get_shape()[-1] == 1

             concat.outputs = tf.tile(concat.outputs, multiples=[1, 1, 1, layer1.outputs.shape[3]])
             concat = ElementwiseLayer([concat, layer1], tf.multiply, name='concat_multiply_SA_{}_{}_{}'.format(i,j,c))

             self.outputs = concat.outputs

        self.all_layers = list(concat.all_layers)
        self.all_params = list(concat.all_params)
        self.all_drop = dict(concat.all_drop)
        self.all_layers.extend( [self.outputs] )

class SA_WithConv_Layer(Layer):
    def __init__(
        self,
        layer1 = None, #input 'Feature' with Layer format[batch,height,width,channel],
        layer2 = None, #
...
class CBAMLayer(Layer):
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            ...
            layer1 = CALayer(layer1, i, j, c)
            layer1 = SALayer(layer1, i, j, c)

            self.outputs = layer1.outputs  #
...
class CBAM_WithConv_Layer(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        ...
        with tf.variable_scope(name) as vs:
            ...
            layer1 = CA_WithConv_Layer(layer1, i, j, c)
            layer1 = SA_WithConv_Layer(layer1, i, j, c)
...
class RCSABLayer(Layer):
    def __init__(
        ...
        j = layer3
        with tf.variable_scope(name) as vs:
            channel = x.outputs.get_shape()[-1]
            #split_list = []
            for c in range(cardinality):
                if c < (cardinality//4):
                    #splits = Conv2d(x, channel//(cardinality * 2), (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_{}_{}_{}'.format(i, j, c))
                    splits = Conv2d(x, channel//(cardinality), (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_{}_{}_{}'.format(i, j, c))
                    splits = CBAMLayer(splits, i, j, c)
                    #split_list.append(splits)
                if c >= (cardinality//4):
                    if (c != 0) and (c % (cardinality//4) == 0):
                        kernel_size = kernel_size + 2
                    #splits = Conv2d(x, channel//(cardinality * 2), (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_{}_{}_{}'.format(i, j, c))
                    #splits = Conv2d(splits, channel//(cardinality * 2), (kernel_size, kernel_size), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_x2_{}_{}_{}'.format(i, j, c))
                    splits = Conv2d(x, channel//(cardinality), (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_{}_{}_{}'.format(i, j, c))
                    splits = Conv2d(splits, channel//(cardinality), (kernel_size, kernel_size), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_x2_{}_{}_{}'.format(i, j, c))
                    splits = CBAMLayer(splits, i, j, c)
                    #split_list.append(splits)
                if c == 0:
                    RCSAB_concat_sublayer = splits
                else:
                    RCSAB_concat_sublayer = ConcatLayer([RCSAB_concat_sublayer, splits], concat_dim=3, name='RCSAB_concat_sublayer_{}_{}_{}'.format(i, j, c))

            #RCSAB_concat_sublayer = Conv2d(RCSAB_concat_sublayer, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_concat_sublayer_conv_{}_{}'.format(i, j))
            RCSAB_concat_sublayer = ElementwiseLayer([RCSAB_concat_sublayer, layer1], tf.add, name='RCSAB_concat_sublayer_conv_residual_{}_{}'.format(i,j))
            self.outputs = RCSAB_concat_sublayer.outputs  #

        self.all_layers = list(RCSAB_concat_sublayer.all_layers)
        self.all_params = list(RCSAB_concat_sublayer.all_params)
        self.all_drop = dict(RCSAB_concat_sublayer.all_drop)
        self.all_layers.extend([self.outputs])

class RCSAB_sum_Layer(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        ...
        with tf.variable_scope(name) as vs:
            ...
            for c in range(cardinality):
                if c < (cardinality//4):
                    splits = Conv2d(x, channel//(cardinality), (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_{}_{}_{}'.format(i, j, c))
                    splits = CBAMLayer(splits, i, j, c)
                    splits = Conv2d(splits, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_expand_{}_{}_{}'.format(i, j, c))
                    #split_list.append(splits)
                if c >= (cardinality//4):
                    if (c != 0) and (c % (cardinality//4) == 0):
                        kernel_size = kernel_size + 2
                    ...
                    splits = Conv2d(splits, channel//(cardinality), (kernel_size, kernel_size), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_x2_{}_{}_{}'.format(i, j, c))
                    splits = CBAMLayer(splits, i, j, c)
                    splits = Conv2d(splits, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCSAB_x2_expand_{}_{}_{}'.format(i, j, c))
                    #split_list.append(splits)
                if c == 0:
                    RCSAB_concat_sublayer = splits
                else:
                    RCSAB_concat_sublayer = ElementwiseLayer([RCSAB_concat_sublayer, splits], tf.add, name='RCSAB_x2_expand_sum_{}_{}'.format(i, j)) #

            #RCSAB_concat_sublayer = ConcatLayer([split_list[0], split_list[1]], concat_dim=3, name='RCSAB_concat_sublayer_{}_{}_{}_{}'.format(i, j, k, 1))
            #for sub_layer in range(2, cardinality-1):
            #    RCSAB_concat_sublayer = ConcatLayer([ RCSAB_concat_sublayer, split_list[sub_layer]], concat_dim=3, name='RCSAB_concat_sublayer_{}_{}_{}_{}'.format(i, j, k, sub_layer))
...
class RCBLayer(Layer):
    def __init__(
        ...
        r = 16,
        w_init=tf.variance_scaling_initializer(scale=0.01, seed=7),
        name='RCB', #Residual Cascading Block
    ):
        Layer.__init__(self, name=name)
        x = layer1
        i = layer2
        with tf.variable_scope(name) as vs:
            ...
            RCB_1 = RCSAB_sum_Layer(x, i, 1) # j=1
            RCB_1_c1 = ConcatLayer([RCB_1, x], concat_dim=3, name='RCB_1_c1_{}_{}'.format(i,1)) #j=1
            RCB_1_c1 = Conv2d(RCB_1_c1, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCB_conv1_{}_{}'.format(i, 1)) #j=1

            RCB_2 = RCSAB_sum_Layer(RCB_1_c1, i, 2) #j=2
            RCB_2_c2 = ConcatLayer([RCB_2, RCB_1, x], concat_dim=3, name='RCB_2_c2_{}_{}'.format(i,2)) #j=2
            RCB_2_c2 = Conv2d(RCB_2_c2, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCB_conv2_{}_{}'.format(i, 2)) #j=2

            RCB_3 = RCSAB_sum_Layer(RCB_2_c2, i, 3) #j=3
            RCB_3_c3 = ConcatLayer([RCB_3, RCB_1, RCB_2, x], concat_dim=3, name='RCB_3_c3_{}_{}'.format(i,3)) #j=3
            RCB_3_c3 = Conv2d(RCB_3_c3, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCB_conv3_{}_{}'.format(i, 3)) #j=3

            RCB_4 = RCSAB_sum_Layer(RCB_3_c3, i, 4) #j=4
            RCB_4_c4 = ConcatLayer([RCB_4, RCB_1, RCB_2, RCB_3, x], concat_dim=3, name='RCB_4_c4_{}_{}'.format(i,4)) #j=4
            RCB_4_c4 = Conv2d(RCB_4_c4, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCB_conv4_{}_{}'.format(i, 4)) #j=4

            RCB_4_c4 = ElementwiseLayer([RCB_4_c4, layer1], tf.add, name='RCB_conv4_residual_{}_{}'.format(i,4)) #j=4

            self.outputs = RCB_4_c4.outputs  #

        self.all_layers = list(RCB_4_c4.all_layers)
        self.all_params = list(RCB_4_c4.all_params)
        self.all_drop = dict(RCB_4_c4.all_drop)
        self.all_layers.extend([self.outputs])

class RCB_8_Layer(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        r = 16,
        w_init=tf.variance_scaling_initializer(scale=0.01, seed=7),
        name='RCB_8', #Residual Cascading Block
    ):
        Layer.__init__(self, name=name)
        x = layer1
        i = layer2
        with tf.variable_scope(name) as vs:
            ...
            RCB_5 = RCSAB_sum_Layer(RCB_4_c4, i, 5)  # j=5
            RCB_5_c5 = ConcatLayer([RCB_5, RCB_4, RCB_1, RCB_2, RCB_3, x], concat_dim=3, name='RCB_5_c5_{}_{}'.format(i, 5))  # j=5
            RCB_5_c5 = Conv2d(RCB_5_c5, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCB_conv5_{}_{}'.format(i, 5))  # j=5

            RCB_6 = RCSAB_sum_Layer(RCB_5_c5, i, 6)  # j=6
            RCB_6_c6 = ConcatLayer([RCB_6, RCB_5, RCB_4, RCB_1, RCB_2, RCB_3, x], concat_dim=3, name='RCB_6_c6_{}_{}'.format(i, 6))  # j=6
            RCB_6_c6 = Conv2d(RCB_6_c6, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCB_conv6_{}_{}'.format(i, 6))  # j=6

            #RCB_7 = RCSABLayer(RCB_6_c6, i, 7)  # j=7
            #RCB_7_c7 = ConcatLayer([RCB_7, RCB_6, RCB_5, RCB_4, RCB_1, RCB_2, RCB_3, x], concat_dim=3, name='RCB_7_c7_{}_{}'.format(i, 7))  # j=7
            #RCB_7_c7 = Conv2d(RCB_7_c7, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCB_conv7_{}_{}'.format(i, 7))  # j=7

            #RCB_8 = RCSABLayer(RCB_7_c7, i, 8)  # j=8
            #RCB_8_c8 = ConcatLayer([RCB_8, RCB_7, RCB_6, RCB_5, RCB_4, RCB_1, RCB_2, RCB_3, x], concat_dim=3, name='RCB_8_c8_{}_{}'.format(i, 8))  # j=8
            #RCB_8_c8 = Conv2d(RCB_8_c8, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='RCB_conv8_{}_{}'.format(i, 8))  # j=8

            RCB_6_c6 = ElementwiseLayer([RCB_6_c6, layer1], tf.add, name='RCB_conv8_residual_{}_{}'.format(i,5)) #j=5

            self.outputs = RCB_6_c6.outputs  #

        self.all_layers = list(RCB_6_c6.all_layers)
        self.all_params = list(RCB_6_c6.all_params)
        self.all_drop = dict(RCB_6_c6.all_drop)
        self.all_layers.extend([self.outputs])

class RCB_ShareSkip_Layer(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        sub_layers = 5,
        w_init=tf.variance_scaling_initializer(scale=0.01, seed=7),
        name='RCB_ShareSkip', #Share Skip Connections
    ):
        Layer.__init__(self, name=name)
        x = layer1
        i = layer2
        with tf.variable_scope(name) as vs:
            for j in range(sub_layers):
                x = RCSAB_sum_Layer(x, i, j)
                if j <= sub_layers - 1:
                    x = ElementwiseLayer([x, layer1], tf.add, name='share_skip_add_{}_{}'.format(i,j)) #Share-source Skip Connections
...
class RCB_ShareSkip_Plus_Layer(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        sub_layers = 6,
        w_init=tf.variance_scaling_initializer(scale=0.01, seed=7),
        name='RCB_ShareSkip_Plus', #Share Skip Connections
    ):
        Layer.__init__(self, name=name)
        x = layer1
        i = layer2
        with tf.variable_scope(name) as vs:
            ...
            for j in range(sub_layers):
                ...
                if j == 0:
                    ...
                    rgb_concat_layer = ConcatLayer([rgb_concat_layer, x], concat_dim=3, name='rgb_concat_layer_{}_{}'.format(i,j))
                if j <= sub_layers - 1:
                    x = ElementwiseLayer([x, layer1], tf.add, name='share_skip_add_{}_{}'.format(i,j)) #Share-source Skip Connections

            rgb_concat_layer = Conv2d(rgb_concat_layer, channel, (1, 1), (1, 1), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, name='rgb_concat_layer_conv_{}_{}'.format(i,j))
            rgb_concat_layer = ElementwiseLayer([rgb_concat_layer, layer1], tf.add, name='share_skip_add_{}_{}'.format(i,j)) #Share-source Skip Connections

            self.outputs = rgb_concat_layer.outputs  #

        self.all_layers = list(rgb_concat_layer.all_layers)
        self.all_params = list(rgb_concat_layer.all_params)
        self.all_drop = dict(rgb_concat_layer.all_drop)
        self.all_layers.extend([self.outputs])

class CBAMLayer_test(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        ...
        k = layer4
        with tf.variable_scope(name) as vs:
            dense_conv_1 = Conv2d(x, 32, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='dense_conv_1_{}_{}_{}'.format(i,j,k))
            dense_conv_5 = Conv2d(dense_conv_1, 32, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='dense_conv_5_{}_{}_{}'.format(i,j,k))


            dense_CA = CALayer(dense_conv_5, i, j, k)
            dense_CA_SA = SALayer(dense_CA, i, j, k)

            self.outputs = dense_CA_SA.outputs  #

        self.all_layers = list(dense_CA_SA.all_layers)
        self.all_params = list(dense_CA_SA.all_params)
        self.all_drop = dict(dense_CA_SA.all_drop)
        self.all_layers.extend([self.outputs])

class RCABLayer(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        ...
        j = layer3
        with tf.variable_scope(name) as vs:
            RCAB_block = Conv2d(x, 64, (3, 3), (1, 1), padding='SAME', act=tf.nn.relu, name='RCAB_conv1_{}_{}'.format(i,j))
            RCAB_block = Conv2d(RCAB_block, 64, (3, 3), (1, 1), padding='SAME', act=None, name='RCAB_conv2_{}_{}'.format(i,j))

            RCAB_block_CA = GlobalPoolLayer(RCAB_block, i, j, name='RCAB_CA_Global_{}_{}'.format(i,j))
            RCAB_block_CA = Conv2d(RCAB_block_CA, 4, (1, 1), (1, 1), padding='SAME', act=tf.nn.relu, name='RCAB_CA_Global_conv_{}_{}'.format(i,j))
            RCAB_block_CA = Conv2d(RCAB_block_CA, 64, (1, 1), (1, 1), padding='SAME', act=tf.nn.sigmoid, name='RCAB_CA_Global_conv_conv_{}_{}'.format(i,j))
            RCAB_block_CA.outputs = tf.tile(RCAB_block_CA.outputs, multiples=[1, RCAB_block.outputs.shape[1], RCAB_block.outputs.shape[2], 1])
            RCAB_block_CA = ElementwiseLayer([RCAB_block_CA, RCAB_block], tf.multiply, name='RCAB_CA_Global_conv_conv_element{}_{}'.format(i,j))

            RCAB_block_CA = ElementwiseLayer([RCAB_block_CA, layer1], tf.add, name='dense_final_{}_{}'.format(i,j))
            self.outputs = RCAB_block_CA.outputs  #

        self.all_layers = list(RCAB_block_CA.all_layers)
        self.all_params = list(RCAB_block_CA.all_params)
        self.all_drop = dict(RCAB_block_CA.all_drop)
        self.all_layers.extend([self.outputs])

class DRRCABLayer(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        ...
        j = layer3
        with tf.variable_scope(name) as vs:
            dense_conv_1 = Conv2d(x, 32, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='dense_conv_1_{}_{}'.format(i,j))
            dense_conv_2_in = ConcatLayer([x, dense_conv_1], concat_dim=3, name='dense_conv_2_in_{}_{}'.format(i,j))
            dense_conv_2 = Conv2d(dense_conv_2_in, 32, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='dense_conv_2_{}_{}'.format(i,j))
            dense_conv_3_in = ConcatLayer([x, dense_conv_1, dense_conv_2], concat_dim=3, name='dense_conv_3_in_{}_{}'.format(i,j))
            dense_conv_3 = Conv2d(dense_conv_3_in, 32, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='dense_conv_3_{}_{}'.format(i,j))
            dense_conv_4_in = ConcatLayer([x, dense_conv_1, dense_conv_2, dense_conv_3], concat_dim=3, name='dense_conv_4_in_{}_{}'.format(i,j))
            dense_conv_4 = Conv2d(dense_conv_4_in, 32, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='dense_conv_4_{}_{}'.format(i,j))
            dense_conv_5_in = ConcatLayer([x, dense_conv_1, dense_conv_2, dense_conv_3, dense_conv_4], concat_dim=3, name='dense_conv_5_in_{}_{}'.format(i,j))
            dense_conv_5 = Conv2d(dense_conv_5_in, 64, (1, 1), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='dense_conv_5_{}_{}'.format(i,j))

            dense_conv_5_CA_Global = GlobalPoolLayer(dense_conv_5, i, j, name='dense_conv_5_CA_Global_{}_{}'.format(i,j))
            dense_conv_5_CA_Global_conv = Conv2d(dense_conv_5_CA_Global, 4, (1, 1), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='dense_conv_5_CA_Global_conv_{}_{}'.format(i,j))
            dense_conv_5_CA_Global_conv_conv = Conv2d(dense_conv_5_CA_Global_conv, 64, (1, 1), (1, 1), padding='SAME', act=tf.nn.sigmoid, name='dense_conv_5_CA_Global_conv_conv_{}_{}'.format(i,j))
            dense_conv_5_CA_Global_conv_conv.outputs = tf.tile(dense_conv_5_CA_Global_conv_conv.outputs, multiples=[1, dense_conv_5.outputs.shape[1], dense_conv_5.outputs.shape[2], 1])
            dense_conv_5_CA_Global_conv_conv_element = ElementwiseLayer([dense_conv_5, dense_conv_5_CA_Global_conv_conv], tf.multiply, name='dense_conv_5_CA_Global_conv_conv_element{}_{}'.format(i,j))

            dense_final = ElementwiseLayer([x, dense_conv_5_CA_Global_conv_conv_element], tf.add, name='dense_final_{}_{}'.format(i,j))
            self.outputs = dense_final.outputs  #

        self.all_layers = list(dense_final.all_layers)
        self.all_params = list(dense_final.all_params)
        self.all_drop = dict(dense_final.all_drop)
        self.all_layers.extend([self.outputs])

class DRRBLayer(Layer):
    def __init__(
        self,
        layer1=None,
        layer2=None,
        ...
        with tf.variable_scope(name) as vs:
            ...
            dense_final = ElementwiseLayer([x, dense_conv_5], tf.add, name='dense_final_{}_{}'.format(i,j))
            self.outputs = dense_final.outputs  #
...
class BilinearInterpLayer(Layer):
    def __init__(
        self,
        layer1 = None, #input 'im' with Layer format,
        layer2 = None, #input 'coordinates' with Layer format,
        warp_type = 'bilinear',
        #warp = warp_type
        #self.name = name
        name = 'Bilinear_Interp_from_DVF',
    ):
    #def __call__(self, x, flow):
        # expect shape
        # x:(#batch, height, width, #channel)
        ...
        Layer.__init__(self, name=name)
        x = layer1.outputs
        Coordinates = layer2.outputs
        coordinate_x = Coordinates[:, :, :, 0]
        coordinate_y = Coordinates[:, :, :, 1]
        #self.inputs = layer.outputs
        with tf.variable_scope(name) as vs:
            assert warp_type in ['nearest', 'bilinear']
            if warp_type == 'nearest':
                self.outputs = nearest_warp(x, flow)
            else:
                self.outputs = bilinear_interp(x, coordinate_x, coordinate_y, name)

        self.all_layers = list(layer2.all_layers)
        self.all_params = list(layer2.all_params)
        self.all_drop = dict(layer2.all_drop)
        self.all_layers.extend( [self.outputs] )

# Warping layer ---------------------------------
def get_grid(x):
    #batch_size, height, width, filters = tf.unstack(tf.shape(x))
    batch_size, height, width, filters = x.get_shape()
...
def bilinear_warp(im, flow):

    #num_batch, height, width, channels = tf.unstack(tf.shape(im))
    num_batch, height, width, channels = im.get_shape()
...
class WarpingLayer(Layer):
    def __init__(
        self,
        layer1 = None, #input 'Feature' with Layer format,
        layer2 = None, #input 'flow' with Tensor format,
        warp_type = 'bilinear',
        #warp = warp_type
        #self.name = name
        name = 'warping',
    ...
    #def __call__(self, x, flow):
        ...
        with tf.variable_scope(name) as vs:
            ...
            if warp_type == 'nearest':
                ...
                self.outputs = bilinear_warp(x, flow)
...
class CostVolumeLayer(Layer):
    ...
    #def __call__(self, features_0, features_0from1):
        Layer.__init__(self, name=name)
        features_0 = layer1.outputs
        features_0from1 = layer2.outputs
        #self.inputs = layer.outputs
        with tf.variable_scope(name) as vs:
            #b, h, w, f = tf.unstack(tf.shape(features_0))
            b, h, w, f = features_0.get_shape()
...
class OpticalFlowEstimatorLayer(Layer):
    def __init__(
        self, 
        layer1 = None,
        layer2= None,
    ...
    #def __call__(self, cost, x, flow):
        ...
        with tf.variable_scope(self.name) as vs:
            flow = InputLayer(tf.cast(flow, dtype = tf.float32), name='tensor2layer/%s' % index)
            x = ConcatLayer([cost, x, flow], concat_dim=3, name='opticalflow_concat/%s' % index)
            x = Conv2d(x, 128, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='n128k3s1_l1/%s' % index)
            x = Conv2d(x, 128, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='n128k3s1_l2/%s' % index)
            x = Conv2d(x, 96, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='n96k3s1_l1/%s' % index)
            x = Conv2d(x, 64, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='n64k3s1_l1/%s' % index)
            feature = Conv2d(x, 32, (3, 3), (1, 1), padding='SAME', act=tf.nn.leaky_relu, name='n32k3s1_l1/%s' % index)
            flow = Conv2d(feature, 2, (3, 3), (1, 1), padding='SAME', act=tf.identity, name='n2k3s1_l1/%s' % index)
            Feature_Flow_final = ConcatLayer([feature, flow], concat_dim=3, name='feature_flow_final_concat/%s' % index)
            self.outputs = Feature_Flow_final.outputs   # x:processed feature, w:processed flow

        self.all_layers = list(Feature_Flow_final.all_layers)
        self.all_params = list(Feature_Flow_final.all_params)
        self.all_drop = dict(Feature_Flow_final.all_drop)
        self.all_layers.extend( [self.outputs] )

class ContextNetworkLayer(Layer):
    """ Context layer """
    def __init__(
        self,
        layer1 = None,
    ...
    #def __call__(self, feature_flow):
        """
        Args:
        ...
        """
        Layer.__init__(self, name=name)
        feature_flow = layer1.outputs
        x = layer1
        index = layer2
        with tf.variable_scope(self.name) as vs:
            #x = tf.concat([flows, features], axis = 3)
            x = AtrousConv2dLayer(x, 128, (3, 3), 1, padding='SAME', act=tf.nn.leaky_relu, name='Dila128k3s1_l1/%s' % index)
            x = AtrousConv2dLayer(x, 128, (3, 3), 2, padding='SAME', act=tf.nn.leaky_relu, name='Dila128k3s1_l2/%s' % index)
            x = AtrousConv2dLayer(x, 128, (3, 3), 4, padding='SAME', act=tf.nn.leaky_relu, name='Dila128k3s1_l3/%s' % index)
            x = AtrousConv2dLayer(x, 96, (3, 3), 8, padding='SAME', act=tf.nn.leaky_relu, name='Dila128k3s1_l4/%s' % index)
            x = AtrousConv2dLayer(x, 64, (3, 3), 16, padding='SAME', act=tf.nn.leaky_relu, name='Dila128k3s1_l5/%s' % index)
            x = AtrousConv2dLayer(x, 32, (3, 3), 1, padding='SAME', act=tf.nn.leaky_relu, name='Dila128k3s1_l6/%s' % index)
            x = AtrousConv2dLayer(x, 2, (3, 3), 1, padding='SAME', act=tf.identity, name='Dila128k3s1_l7/%s' % index)

            self.outputs = feature_flow[:,:,:,32:34] + layer1.outputs
...
def Conv2d(net, n_filter=32, filter_size=(3, 3), strides=(1, 1), act = None,
        ...
        W_init_args = {}, b_init_args = {}, use_cudnn_on_gpu = None, data_format = None,name ='conv2d',reuse=False):
