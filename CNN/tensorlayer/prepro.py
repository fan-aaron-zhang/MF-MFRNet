...
def threading_data2(data1=None, data2=None, fn=None, **kwargs):
    """Return a batch of result by given data.
    Usually be used for data augmentation."""

    def apply_fn(data1,data2,kwargs):
        fn(data1,data2,**kwargs)
...
# mariana
def imresize_multi(x, size=[100, 100], interp='bilinear', mode=None):
    """Resize an image by given output size and method. Warning, this function
    will rescale the value to [0, 255].
    ...
    for idxIm in range(len(x)):
        ...
        if im.ndim == 3:
            ...
            imY = im[:,:,0]
            imY_up = np.repeat(np.repeat(imY, 2, axis=0), 2, axis=1)
